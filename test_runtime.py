...
class webnav_agent(RoutedAgent):
    ...
    async def handle_goal_message(self, message: initial_goal_message, ctx: MessageContext) -> None:
        self._chat_history.append("Message from Orchestrator")
        ...
        self._chat_history.append(webnav_tool_message(content=UserMessage(content=model_completion.content, source=self.id.type)))
        await self.publish_message(webnav_tool_message(content=UserMessage(content=model_completion.content, source=self.id.type)), topic_id=DefaultTopicId(type="nav"))
